...
writer = SummaryWriter("./stage_II_logs")
...
adversarial_scenarios = ["reverse", "reverse", "reverse", "reverse"]
...
def run(env: BEVENV, policy, policy_path, action_bound: list, optimizer, start_episode = 0):
    ...
    for id in range(start_episode, MAX_EPISODES):
        ...
        # we select one adversarial scenarios randomly
        scenarios_type = random.choice(adversarial_scenarios)
        # scenarios_type = "crossing" # tested type: stop + reverse + merge + crossing
        spawn_npc_subprocess = subprocess.Popen(["python", "Scenarios/adversary_scenarios_generator_v3.py", scenarios_type])
        time.sleep(1)
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            env.render()
            # r, terminal, result = env.reward_mark_XIV(step)  # env.reward(step)
            r, terminal, result = env.reward_mark_XIV_reverse(step) # reverse reward function .
            ep_reward += r
            global_step += 1
        ...
        if env.index == 0:
            ...
            name = f"TOWN02_OURS_STAGEII_{id}_{ep_reward}"
            if id % 10 == 0:
                torch.save({
                    "episode_id": id,
                    "episode_reward": ep_reward,
...
if __name__ == "__main__":
    ...
    policy_checkpoints_pth = "checkpoints_II/"
    last_epoch_checkpoint = policy_checkpoints_pth + "last.pth"


    # 3. For training the stage II, load the best state dict from stage I.
    # best_pre_checkpoint_path = "checkpoints_I/best.pth"
    best_pre_checkpoint_path = "./5429_best.pth"
    assert os.path.exists(best_pre_checkpoint_path) == True
    best_checkpoint = torch.load(best_pre_checkpoint_path, map_location="cuda:0")
    print("##################################################")
    print("####### Loading Best Curriculum State Dict #######")
    print("##################################################")
