...
scenarios_route = {
    ...
    # modified
    "Town03_route3": [[44.4, -7.2, 0.22251926362514496,
                       0.0, 0.0, -1.0],
                      [-5.504673480987549, 80.81372833251953, 0.002910823794081807,
                       -0.0024724396813258945, -0.0024310802464402865, -0.7138043727337419, 0.7003365579184387]],
    ...
    "Town03_route5": [[44.4, -7.2, 0.22251926362514496,
                       0.0, 0.0, -1.0],
                      [-5.504673480987549, 80.81372833251953, 0.002910823794081807,
                       -0.0024724396813258945, -0.0024310802464402865, -0.7138043727337419, 0.7003365579184387]],
    ...
    "Town03_route8": [[44.4, -7.2, 0.22251926362514496,
                       0.0, 0.0, -1.0],
                      [-5.504673480987549, 80.81372833251953, 0.002910823794081807,
                       -0.0024724396813258945, -0.0024310802464402865, -0.7138043727337419, 0.7003365579184387]],
    ...
    "Town03_route17": [[44.4, -7.2, 0.22251926362514496,
                       0.0, 0.0, -1.0],
                      [-5.504673480987549, 80.81372833251953, 0.002910823794081807,
                       -0.0024724396813258945, -0.0024310802464402865, -0.7138043727337419, 0.7003365579184387]],
...
class BEVENV:
    def __init__(self, bev_range, bev_img_size, ego_vehicle_dimensions, target_velocity, scenarios_random=True):
        ...
        # Route
        self.scenarios_route = scenarios_route["Town03_route1"] # Town02_route_11 Town01_route1 Town07_route1
        ...
        self.run_off_dis_threshold = 2.5          # m, max lateral distance threshold .
    ...
    def render(self, step, store=False):
        """
        ...
        if store and render_img is not None:
            cv2.imwrite(f"case_library/case_IV/{str(step).zfill(6)}.png", render_img)
    ...
    def reward_stcr_risk_aware(self, step):
        ...
        print(f"Risk spatial: {curr_vel * (1 - self.cad_ranges[191]/15)}")
        print(f"Risk run-off: {curr_vel * (1 - np.cos(math.radians(curr_diff_angle)))}")
        print(f"Risk temporal: {temporal_risk_index}")
    ...
    def test_reward(self, step):
        ...
        if curr_vel < 0.3 and empty_flag == True and self.get_traffic_light_state() == False:  # and len(self.surrounding_objs) == 0
            self.invalid_stop_frames += 1
        else:
            self.invalid_stop_frames = 0
